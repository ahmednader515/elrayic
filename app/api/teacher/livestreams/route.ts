...
export async function GET(req: Request) {
    try {
        ...
        const livestreamsWithCount = livestreams.map((ls: typeof livestreams[0]) => {
            let isExpired = false;
...
export async function POST(req: Request) {
    try {
        ...
        if (!isStaff(user?.role)) {
            return NextResponse.json({ error: "Unauthorized to create livestream for this course" }, { status: 403 });
        }
